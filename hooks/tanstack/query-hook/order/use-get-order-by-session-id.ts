import {  getOrderRequestsByTableId } from "@/utils/actions/order/order.get";
import { keepPreviousData, useQuery } from "@tanstack/react-query";

export const fetchOrderById = async (id : string) => {
  const res = await getOrderRequestsByTableId(id);
  return res;
}

export const useGetOrderRequestsBySessionId = (id : string, pooling ?: boolean) => {
  return useQuery({
    queryKey: ["get-orders-by-table-session", id],
