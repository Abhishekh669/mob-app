...
export default function OrdersStatusPage() {
  ...
  // ── Helpers ──────────────────────────────────────────────────────────────────
  /** An order is "finished" only when every non-cancelled item is `completed`.
   *  Items in `progress`, `approved`, or `not-approved` do NOT count as done. */
  const isOrderFinished = (o: CustomerOrderRequest) => {
    const active = o.order_items.filter((i) => i.status !== "cancelled");
    return active.length > 0 && active.every((i) => i.status === "completed");
  };

  /** Per-order progress: completed / (all non-cancelled).
   *  Cooking / approved / pending contribute 0% — only `completed` moves the bar. */
  ...
  const stats = useMemo(() => {
    ...
    const completedOrders = orders.filter(isOrderFinished).length;
    const pendingOrders = orders.filter((o) => !isOrderFinished(o)).length;
    return {
      completedOrders,
      pendingOrders,
  ...
  const filteredOrders = useMemo(() => {
    ...
    return orders.filter((order) => {
      // Hide fully-finished orders unless user is explicitly filtering for completed
      if (statusFilter !== "completed" && isOrderFinished(order)) return false;
  ...
  const doUpdate = async (order: CustomerOrderRequest, item: OrderItemType, newStatus: orderStatus) => {
    ...
    try {
      const data : UpdateOrderItemType = {
        status: newStatus,
        order_id : order.id,
  ...
  return (
    <View style={s.page}>
      ...
      <View style={s.header}>
        ...
        {/* ── Orders overview — completed vs pending counts only ── */}
        <View style={s.overviewCard}>
          <Text style={s.overviewLabel}>Orders Overview</Text>
          <View style={s.overviewRow}>
            {/* Completed orders */}
            <View style={[s.overviewBlock, { backgroundColor: C.greenSoft, borderColor: C.green + "33" }]}>
              <View style={s.overviewBlockTop}>
                <Ionicons name="checkmark-done-circle" size={20} color={C.green} />
                <Text style={[s.overviewCount, { color: C.green }]}>{stats.completedOrders}</Text>
              </View>
              <Text style={[s.overviewBlockLabel, { color: C.green }]}>Completed</Text>
            </View>

            {/* Pending orders */}
            <View style={[s.overviewBlock, { backgroundColor: C.amberSoft, borderColor: C.amber + "33" }]}>
              <View style={s.overviewBlockTop}>
                <Ionicons name="time" size={20} color={C.amber} />
                <Text style={[s.overviewCount, { color: C.amber }]}>{stats.pendingOrders}</Text>
              </View>
              <Text style={[s.overviewBlockLabel, { color: C.amber }]}>In Progress</Text>
            </View>

            {/* Total */}
            <View style={[s.overviewBlock, { backgroundColor: C.graySoft, borderColor: C.divider }]}>
              <View style={s.overviewBlockTop}>
                <Ionicons name="receipt-outline" size={20} color={C.ink3} />
                <Text style={[s.overviewCount, { color: C.ink }]}>{stats.totalOrders}</Text>
              </View>
              <Text style={[s.overviewBlockLabel, { color: C.ink3 }]}>Total</Text>
            </View>
          </View>
        </View>
      ...
      >
        {filteredOrders.length === 0 ? (
          ...
          filteredOrders.map((order) => {
            ...
            // Per-order progress: ONLY completed items count, not cooking/approved
            ...
            const totalItems = order.order_items.filter((i) => i.status !== "cancelled").length;
              ...
              <View key={order.id} style={s.card}>
                ...
                >
                  ...
                  <View style={s.cardHeaderMid}>
                    ...
                    {/* Per-order progress bar with item count label */}
                    <View style={s.cardHeaderMeta}>
                      <MiniProgressBar pct={pct} />
                      <Text style={s.cardPct}>{doneCt}/{totalItems}</Text>
                    </View>
...
const s = StyleSheet.create({
  ...
  // Overview card — order counts only, no misleading %
  overviewCard:      { backgroundColor: C.graySoft, borderRadius: 14, padding: 14, marginBottom: 14 },
  overviewLabel:     { fontSize: 11, color: C.ink3, fontWeight: "600", textTransform: "uppercase", letterSpacing: 0.6, marginBottom: 10 },
  overviewRow:       { flexDirection: "row", gap: 10 },
  overviewBlock:     { flex: 1, borderRadius: 12, borderWidth: 1, padding: 12, alignItems: "flex-start" },
  overviewBlockTop:  { flexDirection: "row", alignItems: "center", gap: 6, marginBottom: 4 },
  overviewCount:     { fontSize: 26, fontWeight: "700", letterSpacing: -0.5 },
  overviewBlockLabel:{ fontSize: 11, fontWeight: "600" },
  ...
  // Shows "2/5" (done/total) instead of a misleading overall %
